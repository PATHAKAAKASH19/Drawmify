...
export default function useShapeTool(
    ...
    useEffect(() => {
        ...
        const startDrawing = (e) => {
            ...
            if (shapeName !== "pencil") {
                setInitialPos(mousePos);
                 ...
                 const point = {
                   x: (mousePos.x - offset?.x * scale + scaleOffset.x) / scale,
                   y: (mousePos.y - offset?.y * scale + scaleOffset.y) / scale,
                 };
                 setPoints([point]);
            }
        ...
        const finishDrawing = (e) => {
            e.preventDefault();
            shapeName !== "pencil"?addShapes(shape) : addShapes({shapeName:shapeName, points: points})
            setIsDrawing(false)
            setInitialPos(null)
        }
    ...
    }, [canvasRef, contextRef, scale, scaleOffset, shapesData,addShapes, isDrawing, initialPos,offset, shapeName, points])
